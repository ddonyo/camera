        ...
        return new Promise((resolve) => {
            ...
            const tick = () => {
                ...
                if (p >= 1) {
                    resolve({
                        status: 'succeeded',
                        resultUrl: MOCK_RESULT_MAP[garmentId] || './resources/vton/placeholder.jpg',
                    });
                } else {
...
async function createJob({ personRef, garmentId, options }, onProgress) {
    ...
    console.log(
        `[VTON Service] Sending API request with mode: ${options?.vton_mode || 'balanced'}`
    );
